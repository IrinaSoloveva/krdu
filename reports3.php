...
switch($u['prava']) {
	...
	default: if($u['kod']!=3)die('no privilegies'); break;
}
...
if(isset($_POST['sendReport']) && (int)$_POST['sendReport']===1) {
	if($u['prava'] == 255)
		if($t_sogl['soglasovano']==4)
			mysqli_query($q, "UPDATE `bd_3`.`sogl` SET soglasovano2=2,u6=".$u['n'].",date6='".date('d.m.Y H:i:s')."',date6='".date('d.m.Y H:i:s')."' WHERE t_name='$t_name'");
		else
			mysqli_query($q, "UPDATE `bd_3`.`sogl` SET soglasovano=2,u2=".$u['n'].",date='".date('d.m.Y H:i:s')."',date2='".date('d.m.Y H:i:s')."' WHERE t_name='$t_name'");
	elseif($u['prava'] == 0)
		if($t_sogl['soglasovano']==4)
			mysqli_query($q, "UPDATE `bd_3`.`sogl` SET soglasovano2=1,date5='".date('d.m.Y H:i:s')."' WHERE t_name='$t_name'");
		else
			mysqli_query($q, "UPDATE `bd_3`.`sogl` SET soglasovano=1,date='".date('d.m.Y H:i:s')."' WHERE t_name='$t_name'");
	exit;
}
...
elseif(isset($_POST['sendReport']) && (int)$_POST['sendReport']===2 && $u['prava']==255) {
	if($t_sogl['soglasovano']==4)
		mysqli_query($q, "UPDATE `bd_3`.`sogl` SET soglasovano2=2,u6=".$u['n'].",date6='".date('d.m.Y H:i:s')."' WHERE t_name='$t_name'");
	else
		mysqli_query($q, "UPDATE `bd_3`.`sogl` SET soglasovano=2,u2=".$u['n'].",date2='".date('d.m.Y H:i:s')."' WHERE t_name='$t_name'");
	exit;
}

// согласование метОдом + отправка на утверждение 1-му заму
elseif(isset($_POST['sendReport']) && (int)$_POST['sendReport']===3 && $u['kod']==3) {
	if($t_user['prava']==255)
	if($t_sogl['soglasovano']==4)
		echo mysqli_query($q, "UPDATE `bd_3`.`sogl` SET soglasovano2=3,u7=".$u['n'].",date7='".date('d.m.Y H:i:s')."' WHERE t_name='$t_name'");
	else
		echo mysqli_query($q, "UPDATE `bd_3`.`sogl` SET soglasovano=3,u3=".$u['n'].",date3='".date('d.m.Y H:i:s')."' WHERE t_name='$t_name'");
	else
	if($t_sogl['soglasovano']==4)
		echo mysqli_query($q, "UPDATE `bd_3`.`sogl` SET soglasovano2=3,u7=".$u['n'].",date7='".date('d.m.Y H:i:s')."' WHERE t_name='$t_name'");
	else
		echo mysqli_query($q, "UPDATE `bd_3`.`sogl` SET soglasovano=4,u4=".$u['n'].",date4='".date('d.m.Y H:i:s')."' WHERE t_name='$t_name'");
	exit;
}
...
elseif(isset($_POST['sendComment']) && (int)$_POST['sendComment']===1 && strlen(trim((string)$_POST['comment']))>0) {
	if (isset($_POST['toTeacher']) && (int)$_POST['toTeacher']===1 && $u['prava']>0) {
		echo mysqli_query($q, "INSERT INTO `bd_3`.`comments` (`t_name`,`date`,`n`,`text`) VALUES ('$t_name','".date('d.m.Y H:i:s')."',".$u['n'].",'Таблица отправлена на доработку.\r\n".htmlspecialchars(trim((string)$_POST['comment']))."')");
		if($t_sogl['soglasovano']==4)
			echo mysqli_query($q, "UPDATE `bd_3`.`sogl` SET soglasovano2=0,date5=null,date6=null,date7=null,u6=null,u7=null WHERE t_name='$t_name'");
		else
			echo mysqli_query($q, "UPDATE `bd_3`.`sogl` SET soglasovano=0,date=null,date2=null,date3=null,date4=null,u2=null,u3=null,u4=null WHERE t_name='$t_name'");
	} else {
		echo mysqli_query($q, "INSERT INTO `bd_3`.`comments` (`t_name`,`date`,`n`,`text`) VALUES ('$t_name','".date('d.m.Y H:i:s')."',".$u['n'].",'".htmlspecialchars(trim((string)$_POST['comment']))."')");
	}
	exit;
}
...
elseif(isset($_POST['saveReport']) && (int)$_POST['saveReport']===1) {
	mysqli_query($q, 'DROP TABLE if EXISTS `bd_3`.`'.$t_name.'`');
	mysqli_query($q, 'CREATE TABLE `bd_3`.`'.$t_name.'` (`n` VARCHAR(8) NOT NULL,`naim` VARCHAR(200),`plan` VARCHAR(8) NOT NULL DEFAULT \'0\',`fakt` VARCHAR(8),`srok` VARCHAR(50),`otmetka` VARCHAR(500)) ENGINE = InnoDB');
	...
	for ($j=0; $j<count($rows)-1; $j++) {
		...
		$fakt += $r[3];
		$r2 = implode("','", $r);
		mysqli_query($q, 'INSERT INTO `bd_3`.`'.$t_name."` (`n`,`naim`,`plan`,`fakt`,`srok`,`otmetka`) VALUES ('".$r2."')");
	}
	if($t_sogl['date4']) {
		mysqli_query($q, "UPDATE `bd_3`.`sogl` SET date0='".date('d.m.Y H:i:s')."',plan='$plan',fakt='$fakt',date5=null,date6=null,date7=null,soglasovano2=null WHERE t_name='$t_name'");
	} else {
		if($t_sogl)
			mysqli_query($q, "DELETE FROM `bd_3`.`sogl` WHERE t_name='$t_name'");
		mysqli_query($q, "INSERT INTO `bd_3`.`sogl` (`t_name`,`date0`,`plan`,`fakt`,`soglasovano`) VALUES ('$t_name','".date('d.m.Y H:i:s')."','$plan','$fakt',0)");
	}
	echo 'Сохранено!';
	exit;
}
		...
		<div class="main">
          ...
          <span class="align-middle">КРу МВД России</span>
        	...
        	<div class="den">
                ...
                <img src="profile_photos/<?=$u['n']?>.jpg?" class="avatar img-fluid rounded-circle mr-1" alt> <span style="color: #fff;"><?=fio_cut()?></span>
						</li>
					</ul>
				</div>
			</nav>
...
<?}?>
                                                ...
                                                <tbody id="tb" kol="0">
<?
$vid = array('','1.1 Выполнение НИР по планам кафедры (за исключением работ, направленных на подготовку учебников, учебных пособий, лекций и т.д.)','1.2 Выполнение НИР по планам вуза (за исключением работ, направленных на подготовку учебников, учебных пособий, лекций и т.д.)','1.3 Выполнение НИР по планам МВД России (за исключением работ, направленных на подготовку учебников, учебных пособий, лекций и т.д.)','2.1 Написание нового учебника, монографии','2.2 Переиздание учебника','2.3 Написание нового учебного пособия, курса лекций, практикума, справочника','2.4 Написание лекций для издания','2.5 Переиздание лекций','2.6 Написание научных статей докладов, сообщений для научных конференций и семинаров','3 Внедрение научных разработок в практическую деятельность ОВД и его авторское сопровождение','4 Научное редактирование рабочих учебных программ, учебников, учебных пособий, монографий, курсов лекций, научных трудов и т.д.','5 Рецензирование РУП, учебно-методических материалов, учебников, учебных пособий, монографий, курсов лекций, научных трудов, диссертаций, авторефератов, проектов, НПА и т.д.','6 Научная командировка (стажировка)','7.1 Участие в работе ученого совета образовательного учреждения','7.2.1 Участие в работе диссовета в качетсве председателя, заместителя председателя, секретаря','7.2.2 Членство в диссовете','7.3 Участие в работе научно-методического и научно-технического советов МВД России','8 Участие в работе научных конференций, семинаров и т.п.','9 Проведение научных консультаций с докторантом','10.1 Руководство слушательским научным обществом','10.2 Руководство научным кружком;','10.3 Руководство НИР слушателя (НИРС) с представлением слушателем научных отчетов, докладов, статей, рефератов и т.п.','11.1 Разработка кандидатского диссертационного исследования, утвержденного советом образовательного учреждения','11.2 Разработка докторского диссертационного исследования, утвержденного советом образовательного учреждения','12 Организационно-методическая работа','13.1 Участие в представительских мероприятиях: участие в работе Совета по научной деятельности','13.2 Участие в представительских мероприятиях: работа учебно-методического совета','13.3 Участие в представительских мероприятиях: заседания кафедры','13.4 Участие в представительских мероприятиях: заседания МСК');
$norma = array('','до 150','до 300','до 450','до 20 за 1 п.л.','до 50','до 80','до 80','до 70','до 70','до 30','до 18','до 6','до 6 в день','до 30','до 20 за 1 заседание диссовета','до 10 за 1 заседание диссовета','до 30','до 6 в день','до 50','до 60','до 60','до 15','до 200','до 400','до 20','до 10','до 20','до 20','до 20');
$res = mysqli_query($q, 'SELECT * from `bd_3`.`'.$t_name.'`');
...
<option value="1">1.1 Выполнение НИР по планам кафедры (за исключением работ, направленных на подготовку учебников, учебных пособий, лекций и т.д.)</option>
<option value="2">1.2 Выполнение НИР по планам вуза (за исключением работ, направленных на подготовку учебников, учебных пособий, лекций и т.д.)</option>
<option value="3">1.3 Выполнение НИР по планам МВД России (за исключением работ, направленных на подготовку учебников, учебных пособий, лекций и т.д.)</option>
<option value="4">2.1 Написание нового учебника, монографии</option>
<option value="5">2.2 Переиздание учебника</option>
<option value="6">2.3 Написание нового учебного пособия, курса лекций, практикума, справочника</option>
<option value="7">2.4 Написание лекций для издания</option>
<option value="8">2.5 Переиздание лекций</option>
<option value="9">2.6 Написание научных статей докладов, сообщений для научных конференций и семинаров</option>
<option value="10">3 Внедрение научных разработок в практическую деятельность ОВД и его авторское сопровождение</option>
<option value="11">4 Научное редактирование рабочих учебных программ, учебников, учебных пособий, монографий, курсов лекций, научных трудов и т.д.</option>
<option value="12">5 Рецензирование РУП, учебно-методических материалов, учебников, учебных пособий, монографий, курсов лекций, научных трудов, диссертаций, авторефератов, проектов, нормативных актов и т.д.</option>
<option value="13">6 Научная командировка (стажировка)</option>
<option value="14">7.1 Участие в работе ученого совета образовательного учреждения</option>
<option value="15">7.2.1 Участие в работе диссовета в качетсве председателя, заместителя председателя, секретаря</option>
<option value="16">7.2.2 Членство в диссовете</option>
<option value="17">7.3 Участие в работе научно-методического и научно-технического советов МВД России</option>
<option value="18">8 Участие в работе научных конференций, семинаров и т.п.</option>
<option value="19">9 Проведение научных консультаций с докторантом</option>
<option value="20">10.1 Руководство слушательским научным обществом</option>
<option value="21">10.2 Руководство научным кружком;</option>
<option value="22">10.3 Руководство НИР слушателя (НИРС) с представлением слушателем научных отчетов, докладов, статей, рефератов и т.п.</option>
<option value="23">11.1 Разработка кандидатского диссертационного исследования, утвержденного советом образовательного учреждения</option>
<option value="24">11.2 Разработка докторского диссертационного исследования, утвержденного советом образовательного учреждения</option>
<option value="25">12 Организационно-методическая работа</option>
<option value="26">13.1 Участие в представительских мероприятиях: участие в работе Совета по научной деятельности</option>
<option value="27">13.2 Участие в представительских мероприятиях: работа учебно-методического совета</option>
<option value="28">13.3 Участие в представительских мероприятиях: заседания кафедры</option>
<option value="29">13.4 Участие в представительских мероприятиях: заседания МСК</option>
</select>
...
<?}?>
<?if($u['kod']==3 && ($t_sogl['soglasovano']==2 || $t_sogl['soglasovano2']==2)) {?>
...
<?
$res = mysqli_query($q, 'SELECT * from `bd_3`.`comments` WHERE t_name=\''.$t_name.'\'');
for($i=0; $i<$res->num_rows; $i++) {
	$r = mysqli_fetch_assoc($res);
...
db_all = ['1.1 Выполнение НИР по планам кафедры (за исключением работ, направленных на подготовку учебников, учебных пособий, лекций и т.д.)','1.2 Выполнение НИР по планам вуза (за исключением работ, направленных на подготовку учебников, учебных пособий, лекций и т.д.)','1.3 Выполнение НИР по планам МВД России (за исключением работ, направленных на подготовку учебников, учебных пособий, лекций и т.д.)','2.1 Написание нового учебника, монографии','2.2 Переиздание учебника','2.3 Написание нового учебного пособия, курса лекций, практикума, справочника','2.4 Написание лекций для издания','2.5 Переиздание лекций','2.6 Написание научных статей докладов, сообщений для научных конференций и семинаров','3 Внедрение научных разработок в практическую деятельность ОВД и его авторское сопровождение','4 Научное редактирование рабочих учебных программ, учебников, учебных пособий, монографий, курсов лекций, научных трудов и т.д.','5 Рецензирование рабочих учебных программ, учебно-методических материалов, учебников, учебных пособий, монографий, курсов лекций, научных трудов, диссертаций, авторефератов, проектов, нормативных актов и т.д.','6 Научная командировка (стажировка)','7.1 Участие в работе ученого совета образовательного учреждения','7.2.1 Участие в работе диссовета в качетсве председателя, заместителя председателя, секретаря','7.2.2 Членство в диссовете','7.3 Участие в работе научно-методического и научно-технического советов МВД России','8 Участие в работе научных конференций, семинаров и т.п.','9 Проведение научных консультаций с докторантом','10.1 Руководство слушательским научным обществом','10.2 Руководство научным кружком;','10.3 Руководство НИР слушателя (НИРС) с представлением слушателем научных отчетов, докладов, статей, рефератов и т.п.','11.1 Разработка кандидатского диссертационного исследования, утвержденного советом образовательного учреждения','11.2 Разработка докторского диссертационного исследования, утвержденного советом образовательного учреждения','12 Организационно-методическая работа','13.1 Участие в представительских мероприятиях: участие в работе Совета по научной деятельности','13.2 Участие в представительских мероприятиях: работа учебно-методического совета','13.3 Участие в представительских мероприятиях: заседания кафедры','13.4 Участие в представительских мероприятиях: заседания МСК'];
db_norma = ['до 150','до 300','до 450','до 20 за 1 п.л.','до 50','до 80','до 80','до 70','до 70','до 30','до 18','до 6','до 6 в день','до 30','до 20 за 1 заседание диссовета','до 10 за 1 заседание диссовета','до 30','до 6 в день','до 50','до 60','до 60','до 15','до 200','до 400','до 20','до 10','до 20','до 20','до 20'];

...
/*
	if (isset($_POST['toTeacher']) && (int)$_POST['toTeacher']===1 && $u['prava']>0) {
		...
		if($t_sogl['soglasovano']==4)
			echo mysqli_query($q, "UPDATE `bd_1`.`sogl` SET soglasovano2=0,date5=null,date6=null,date7=null,u6=null,u7=null WHERE t_name='$t_name'");
		else
			echo mysqli_query($q, "UPDATE `bd_1`.`sogl` SET soglasovano=0,date=null,date2=null,date3=null,date4=null,u2=null,u3=null,u4=null WHERE t_name='$t_name'");
	} else {
...
elseif ((string)$_GET['form']=='1_sem') {
    ...
    for ($i=9; $i<14; $i++) {
        ...
        $sem1->getActiveSheet()->setCellValueByColumnAndRow(21, $i, $row_sum);
    }

    for($i=9; $i<14; $i++) {
        $a1 = $sem1->getActiveSheet()->getCellByColumnAndRow(12, $i)->getValue();
        $a2 = $sem1->getActiveSheet()->getCellByColumnAndRow(21, $i)->getValue();
        $sem1->getActiveSheet()->setCellValueByColumnAndRow(22, $i, $a1+$a2);
    }
...
elseif ((string)$_GET['form']=='year') {
    ...
    $fakt = $year->getActiveSheet()->getCellByColumnAndRow(22,14)->getCalculatedValue();
...
if(file_exists($ff.'.xls'))
	echo '<a href="'.$ff.'.xls'.'">Скачать</a>',' (',(\PhpOffice\PhpSpreadsheet\IOFactory::load($ff.'.xls')->getActiveSheet()->getCell('V14')->getCalculatedValue()),')',$x;
elseif(file_exists($ff.'.xlsx'))
	echo '<a href="'.$ff.'.xlsx'.'">Скачать</a>',' (',(\PhpOffice\PhpSpreadsheet\IOFactory::load($ff.'.xlsx')->getActiveSheet()->getCell('V14')->getCalculatedValue()),')',$x;
elseif(file_exists($ff.'.xlsm'))
	echo '<a href="'.$ff.'.xlsm'.'">Скачать</a>',' (',(\PhpOffice\PhpSpreadsheet\IOFactory::load($ff.'.xlsm')->getActiveSheet()->getCell('V14')->getCalculatedValue()),')',$x;
elseif($u['kod']==1 || $u['prava']==300 || ($u['prava']==255 && $t_user['login']!=$u['login']))
	echo 'Таблица не загружена';
elseif($t_n==6)
				...
				<div class="container-fluid p-0">
                        <div class="row">
                            <div class="col-sm-12">
                                <div class="card">
                                    <div class="card-body">
                                        <h3 class="text-center">Пользователь: <?=$t_user['f'].' '.$t_user['i'].' '.$t_user['o']?></h3>
                                        <h4 class="header-title m-t-0 m-b-30">1.1 Планируемая учебная нагрузка</h4>
                                        <form></form>
                                        <div>
...
?></td>
                                        ...
                                        <h4 class="header-title m-t-0 m-b-30">1.2 Фактически выполненная учебная нагрузка</h4>
                                        <div>
                                            <table class="table m-b-0">
                                                <thead>
                                                    <tr>
                                            ...
                                            elseif($bd[1]['date6'])
                                                if($u['kod']==1)
                                                    ...
                                                    echo '<h4>На согласовании (УО)</h4>';
                                            elseif($bd[1]['date5']) {
                                                if($u['prava']==255)
                                                    echo '<a class="btn btn-success btn-sm" onclick="sendReport(\'2\')" href="#">Согласовать</a>';
                                                else
                                                    echo '<h4>На согласовании (НК)</h4>';
                                            } elseif($bd[1]['date4']) {
                                                if($u['login']==$t_user['login'])
                                                    echo '<a class="btn btn-success btn-sm" onclick="sendReport(\'2\')" href="#">На согласование</a>';
                                                else
                                                    echo '<h4>На заполнении преподавателем</h4>';
                                            } else echo '<h4>План не согласован</h4>'
                                            ?>
                                        </div>
                                    </div>
                                </div><!-- end card -->
